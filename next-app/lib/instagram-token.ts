...
const TOKEN_FILE = path.resolve(process.cwd(), "data/instagram_token.json");
...
async function exchangeToken(
  ...
  if (!appId || !appSecret) {
    console.warn("[instagram-token] INSTAGRAM_APP_ID/SECRET not set — cannot refresh");
    return null;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[instagram-token] exchange failed", res.status, err);
      return null;
    }
...
async function fetchTokenExpiry(token: string): Promise<Date | null> {
  const appId = sanitize(process.env.INSTAGRAM_APP_ID);
  const appSecret = sanitize(process.env.INSTAGRAM_APP_SECRET);
...
export async function getValidToken(): Promise<string | null> {
  ...
  if (!tokenData) {
    const envToken = sanitize(process.env.INSTAGRAM_ACCESS_TOKEN);
    if (!envToken) return null;

    const expiresAt =
      (await fetchTokenExpiry(envToken)) ??
      new Date(Date.now() + 60 * 24 * 60 * 60 * 1000); // fallback 60日
  ...
  if (daysRemaining < REFRESH_THRESHOLD_DAYS) {
    ...
    if (refreshed) {
      ...
      console.info("[instagram-token] token refreshed, new expiry:", newData.expires_at);
      return newData.access_token;
    }
    // 更新失敗 — まだ有効なら使い続ける
    if (daysRemaining > 0) return tokenData.access_token;
    console.error("[instagram-token] token expired and refresh failed");
    return null;
  }
...
export async function forceRefresh(): Promise<{
  ...
  const tokenData = await loadTokenData();
  const currentToken =
    tokenData?.access_token ?? sanitize(process.env.INSTAGRAM_ACCESS_TOKEN);
