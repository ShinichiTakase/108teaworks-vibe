...
async function getLogFile(
  ...
  if (!/^checkout_\d{8}\.csv$/.test(filename)) return null;
...
export default async function AdminCheckoutFilePage({ params }: Props) {
  const { filename } = await params;
    ...
    <main className="min-h-screen bg-washi py-12 px-4">
      <div className="mx-auto max-w-4xl">
        <div className="mb-4 flex items-center gap-4">
          <Link href="/admin/checkout" className="text-sm text-tea hover:underline">
            ← 一覧に戻る
          </Link>
