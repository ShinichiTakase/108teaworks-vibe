...
async function getCheckoutFiles(): Promise<{ name: string; size: number }[]> {
  ...
  try {
    ...
    const csvFiles = entries
      .filter((f) => f.startsWith("checkout_") && f.endsWith(".csv"))
...
export default async function AdminCheckoutPage() {
  const files = await getCheckoutFiles();
    ...
    <main className="min-h-screen bg-washi py-12 px-4">
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-tea-deep">購入ページ表示一覧</h1>
        </div>
        {files.length === 0 ? (
